...
public class App {
  public static void main(String... args) throws Exception {
    ...
    try {
      ...
      ModelBuilder<OutgoingArtifactsModel> modelBuilder =
      ...
      modelBuilder.addProgressListener(
          (ProgressListener)
              progressEvent ->
                  System.err.printf(
                      "event: %s -> %s%n",
                      progressEvent.getClass(), progressEvent.getDescription()));
  ...
  private static File copyInitScript() throws IOException, URISyntaxException {
    ...
    try (BufferedReader reader =
        new BufferedReader(new InputStreamReader(App.class.getResourceAsStream(name)))) {
      reader
          .lines()
          .forEach(
              line -> {
